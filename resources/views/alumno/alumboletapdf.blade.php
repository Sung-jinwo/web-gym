...
<body>
    <div class="header">
        ...
        <div class="address">
            {{$lastMembresia->alumno->sede->sede_direccion}} – El Porvenir<br>
            Trujillo – La Libertad<br>
            {{$lastMembresia->alumno->sede->sede_telefono}}
        </div>
    </div>

    <div class="header">
        <div class="title">Nota de Venta</div>
        <div class="title">NV 001- {{ str_pad($lastMembresia->id_pag ?? '0', 6, '0', STR_PAD_LEFT) }}</div>
    ...
    <div class="client-info">
        <div> <span>Cliente:</span>
            <div class="mostrar">
                {{ $lastMembresia->alumno->alum_nombre }}, {{$lastMembresia->alumno->alum_apellido}}
            </div>
        </div>
        <div><span>Documento:</span>
            <div class="mostrar">
                @if ($lastMembresia->alumno->alum_numDoc)
                    {{$lastMembresia->alumno->alum_numDoc}}
                @else
        ...
        <div><span>Celular:</span>
            <div class="mostrar">
                {{$lastMembresia->alumno->alum_telefo}}
            </div>
        </div>
    </div>
    ...
    <table class="products">
        ...
        <tbody>
        <tr>
            <td>Membresía paquete <br>{{$lastMembresia->membresia->categoria_m->nombre_m}} x {{$lastMembresia->membresia->mem_nomb}}</td>
            <td style="text-align: center">1</td>
            <td style="text-align: right;">S/{{ number_format($lastDetalle->monto, 2) }}</td>
        </tr>
        </tbody>
    </table>


    <div class="total">TOTAL: S/ {{ number_format($lastDetalle->monto, 2) }}</div>
{{--    <div class="total-text">Ciento Sesenta y Cinco con 00/100 soles</div>--}}

    <div class="seller-info">
        <div>
            <span> Lugar De Venta: </span> {{$lastMembresia->alumno->sede->sede_nombre}}
        </div>
        <div> <span>Condición de Pago:</span>  {{$lastMembresia->metodo->tipo_pago}}</div>
    </div>
