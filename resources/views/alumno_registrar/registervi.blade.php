                    ...
                    <tr class="table-row">
                        ...
                        <td class="table-cell actions-cell">
                            <div class="action-buttons">
                                ...
                                <a href="{{ route('registro.edit', $alumno->fkalum) }}" class="btn-icon btn-green" title="Editar Prospecto">
                                    <i class="icono-editar"></i>
                                </a>
                                <a href="{{route('registro.conversar', $alumno)}}" class="btn-icon btn-rous" title="Conversar Prospecto">
                                    <i class="icono-mensaje"></i>
                                </a>
                                <form action="{{ route('registro.estado',  $alumno->alumno) }}" method="POST" class="inline-form">
                                    @csrf
                                    @method('PUT')
                                    @if ($alumno->alumno->alum_estado == 'A')
                                        <button type="submit" class="btn-icon btn-red" title="Inhabilitar Prospecto" onclick="return confirm('¿Estás seguro de desactivar este alumno?')">
                                            <i class="icono-desactivar"></i>
                                        </button>
                                    @elseif($alumno->alumno->alum_estado == 'E')
                                        <button type="submit" class="btn-icon btn-green" title="Reactivar Prospecto" onclick="return confirm('¿Estás seguro de Activar este alumno?')">
                                            <i class="icono-activar"></i>
                                        </button>
                                    @endif
                                </form>
                                @if(auth()->user()->is(\App\Models\User::ROL_ADMIN))
                                    <form action="{{ route('registro.destroy', $alumno->alumno) }}" method="POST" class="inline-form">
                                        @csrf @method('DELETE')
                                        @if ($alumno->alumno->alum_estado == 'E')
                                            <button type="submit" class="btn-icon btn-red" title="Eliminar este Prospecto" onclick="return confirm('¿Estás seguro de eliminar al alumno?')">
